...
public:
  ...
  explicit DetectionEngine(const std::string& model_path) : BasicEngine(model_path) { Validate(); }

  // Loads detection model and specifies EdgeTpu to use.
  //  - 'model_path' : the file path of the model.
  //  - 'device_path' : the device path of EdgeTpu.
  explicit DetectionEngine(const std::string& model_path, const std::string& device_path)
