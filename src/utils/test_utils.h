...
std::vector<std::pair<int, float>> GetTopK(
    const std::vector<float>& scores, float threshold, int top_k);
...
void TestClassification(
    const std::string& model_path, const std::string& image_path, float effective_scale,
    const std::vector<float>& effective_means, bool rgb2bgr, float score_threshold, int k,
    int expected_topk_label);

// Tests a classification model, default not to shuffle the RGB channels.
void TestClassification(
    const std::string& model_path, const std::string& image_path, float effective_scale,
    const std::vector<float>& effective_means, float score_threshold, int k,
    int expected_topk_label);

// Tests a classification model.
void TestClassification(
    const std::string& model_path, const std::string& image_path, float score_threshold, int k,
    int expected_topk_label);

// Tests a classification model. Only checks the top1 result.
void TestClassification(
    const std::string& model_path, const std::string& image_path, float score_threshold,
...
void BenchmarkModelsOnEdgeTpu(const std::vector<std::string>& model_paths, benchmark::State& state);
...
void InferenceStressTest(const std::string& model_path, int runs, int sleep_sec = 0);
