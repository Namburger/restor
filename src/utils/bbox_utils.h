...
struct BoxCornerEncoding {
  ...
  inline std::string DebugString() const {
    return absl::Substitute("ymin=$0,xmin=$1,ymax=$2,xmax=$3", ymin, xmin, ymax, xmax);
  }
};
...
struct DetectionCandidate {
  ...
  float score;
  inline std::string DebugString() const {
    return absl::Substitute("corners={$0},label=$1,score=$2", corners.DebugString(), label, score);
  }
};

// Compare based on id and score only.
inline bool operator==(const DetectionCandidate& x, const DetectionCandidate& y) {
...
struct DetectionCandidateComparator {
  bool operator()(const DetectionCandidate& lhs, const DetectionCandidate& rhs) const {
...
inline float IntersectionOverUnion(const BoxCornerEncoding& box0, const BoxCornerEncoding& box1) {
