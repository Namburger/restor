...
std::vector<uint8_t> RgbToGrayscale(const std::vector<uint8_t>& in, const ImageDims& in_dims) {
  ...
  for (int in_idx = 0, out_idx = 0; in_idx < in.size(); in_idx += in_dims[2], ++out_idx) {
    int r = in[in_idx];
    int g = in[in_idx + 1];
    int b = in[in_idx + 2];
...
std::string GenerateRandomFilePath(const std::string& prefix, const std::string& suffix) {
  return absl::StrCat(std::string(std::tmpnam(nullptr)), "_", prefix, suffix);
}

std::vector<std::vector<float>> TestWithImage(
  ...
  std::vector<uint8_t> input = GetInputFromImage(
      image_path, {input_tensor_shape[1], input_tensor_shape[2], input_tensor_shape[3]});
...
void TestDetection(
    const std::string& model_path, const std::string& image_path,
    const BoxCornerEncoding& expected_box, int expected_label, float score_threshold,
    float iou_threshold) {
  ...
  auto candiates = engine.DetectWithInputTensor(input_tensor, score_threshold, /*top_k=*/1);
...
void TestCatMsCocoDetection(
    const std::string& model_path, float score_threshold, float iou_threshold) {
  TestDetection(
      model_path, TestDataPath("cat.bmp"),
      /*expected_box=*/{0.1, 0.1, 0.7, 1.0},
      /*expected_label=*/16, score_threshold, iou_threshold);
}

void BenchmarkModelsOnEdgeTpu(
    const std::vector<std::string>& model_paths, benchmark::State& state) {
  ...
  for (int model_index = 0; model_index < number_models; ++model_index) {
    ...
    if (model_index == 0) {
      ...
      engine = absl::make_unique<coral::BasicEngine>(model_path, engines[0]->device_path());
    }
...
void BenchmarkModelOnEdgeTpu(const std::string& model_path, benchmark::State& state) {
...
void RepeatabilityTest(const std::string& model_path, int runs) {
  ...
  int error_count = 0;
  std::vector<std::vector<float>> reference_result = engine.RunInference(input_data);
    ...
    for (int i = 0; i < num_outputs; ++i) {
      for (int j = 0; j < result[i].size(); ++j) {
        if (result[i][j] != reference_result[i][j]) {
          VLOG(1) << "[ iteration = " << r << " ] output of tensor " << i << " at position " << j
                  << " differs from reference.\n"
                  << "( output = " << result[i][j] << " reference = " << reference_result[i][j]
                  << " )";
          ++error_count;
        }
      }
...
void InferenceStressTest(const std::string& model_path, int runs, int sleep_sec) {
  BasicEngine engine(model_path);
...
float ComputeIntersectionOverUnion(
    const std::vector<uint8_t>& mask1, const std::vector<uint8_t>& mask2) {
  int isec_area = 0;
  ...
  return static_cast<float>(isec_area) / (mask1.size() + mask2.size() - isec_area);
}

// Tests segmentation models that include ArgMax operator, returns the
// prediction results.
void TestSegmentationWithArgmax(
    const std::string& model_name, const std::string& image_name, const std::string& seg_name,
    int size, float iou_threshold, std::vector<uint8_t>* pred_segmentation) {
  ...
  std::copy(argmax_result.begin(), argmax_result.end(), pred_segmentation->begin());
  ...
  float seg_iou = ComputeIntersectionOverUnion(*pred_segmentation, groundtruth_segmentation);
  LOG(INFO) << seg_iou;
