...
EdgeTpuApiStatus EdgeTpuResourceManager::CreateEdgeTpuResource(
  ...
  auto tpu_context = edgetpu::EdgeTpuManager::GetSingleton()->OpenDevice(type, path, options);
  if (!tpu_context) {
    error_reporter_->Report(absl::Substitute("Error in device opening ($0)!", path));
    return kEdgeTpuApiError;
  }
  resource_map_[path].usage_count = 1;
  resource_map_[path].context = tpu_context;
  // Using `new` to access a non-public constructor.
  (*resource) = absl::WrapUnique(new EdgeTpuResource(path, resource_map_[path].context));
  return kEdgeTpuApiOk;
}

EdgeTpuApiStatus EdgeTpuResourceManager::GetEdgeTpuResource(
    std::unique_ptr<EdgeTpuResource>* resource) {
  const auto& tpu_devices = edgetpu::EdgeTpuManager::GetSingleton()->EnumerateEdgeTpu();
...
EdgeTpuApiStatus EdgeTpuResourceManager::GetEdgeTpuResource(
  ...
  error_reporter_->Report(absl::Substitute("Path $0 does not map to an Edge TPU device.", path));
  return kEdgeTpuApiError;
}
...
EdgeTpuApiStatus EdgeTpuResourceManager::ReclaimEdgeTpuResource(const std::string& path) {
  absl::MutexLock lock(&mu_);
  auto it = resource_map_.find(path);
  if (it != resource_map_.end()) {
    it->second.usage_count--;
    ...
    error_reporter_->Report(absl::Substitute("Trying to reclaim unassigned device: $0.", path));
    return kEdgeTpuApiError;
  }
  return kEdgeTpuApiOk;
}

std::vector<std::string> EdgeTpuResourceManager::ListEdgeTpuPaths(const EdgeTpuState& state) {
  std::vector<std::string> result;
  const auto& edgetpu_devices = edgetpu::EdgeTpuManager::GetSingleton()->EnumerateEdgeTpu();
  switch (state) {
