...
private:
  ...
  EdgeTpuResource(const std::string& path, std::shared_ptr<edgetpu::EdgeTpuContext> context)
...
public:
  ...
  EdgeTpuApiStatus GetEdgeTpuResource(std::unique_ptr<EdgeTpuResource>* resource)
      LOCKS_EXCLUDED(mu_);
  ...
  EdgeTpuApiStatus ReclaimEdgeTpuResource(const std::string& path) LOCKS_EXCLUDED(mu_);
...
private:
  ...
  EdgeTpuApiStatus GetEdgeTpuResourceMultipleDetected(std::unique_ptr<EdgeTpuResource>* resource)
      LOCKS_EXCLUDED(mu_);
