...
protected:
  void SetUp() override {
    ...
    unassigned_devices_ = resource_manager_->ListEdgeTpuPaths(EdgeTpuState::kUnassigned);
    ASSERT_GE(unassigned_devices_.size(), 1);
  }

  void TearDown() override {
    EXPECT_EQ(0, resource_manager_->ListEdgeTpuPaths(EdgeTpuState::kAssigned).size());
  }
...
TEST_F(EdgeTpuResourceManagerTest, GetAllEdgeTpuOnce) {
  std::vector<std::unique_ptr<EdgeTpuResource>> edgetpu_resources(unassigned_devices_.size());
  for (int i = 0; i < unassigned_devices_.size(); ++i) {
    EXPECT_EQ(i, resource_manager_->ListEdgeTpuPaths(EdgeTpuState::kAssigned).size());
    ASSERT_EQ(kEdgeTpuApiOk, resource_manager_->GetEdgeTpuResource(&edgetpu_resources[i]));
  }
}
...
TEST_F(EdgeTpuResourceManagerTest, GetSameTpuContextRepeatedly) {
  ...
  EXPECT_EQ(
      kEdgeTpuApiOk, resource_manager_->GetEdgeTpuResource(unassigned_devices_[0], &edgetpu_once));
  std::unique_ptr<EdgeTpuResource> edgetpu_twice;
  ASSERT_TRUE(edgetpu_once != nullptr);
  EXPECT_EQ(
      kEdgeTpuApiOk, resource_manager_->GetEdgeTpuResource(unassigned_devices_[0], &edgetpu_twice));
  ...
  EXPECT_EQ(
      kEdgeTpuApiOk,
      resource_manager_->GetEdgeTpuResource(unassigned_devices_[0], &edgetpu_thrice));
...
TEST_F(EdgeTpuResourceManagerTest, CheckDevicePath) {
  ...
  EXPECT_EQ(
      kEdgeTpuApiOk,
      resource_manager_->GetEdgeTpuResource(unassigned_devices_[0], &edgetpu_resource));
...
TEST_F(EdgeTpuResourceManagerTest, DeviceNotExistError) {
  std::unique_ptr<EdgeTpuResource> edgetpu_resource;
  EXPECT_EQ(
      kEdgeTpuApiError, resource_manager_->GetEdgeTpuResource("invalid_path", &edgetpu_resource));
  EXPECT_EQ(
      "Path invalid_path does not map to an Edge TPU device.",
      resource_manager_->get_error_message());
}

TEST_F(EdgeTpuResourceManagerTest, ReclaimUnassignedDeviceError) {
  EXPECT_EQ(kEdgeTpuApiError, resource_manager_->ReclaimEdgeTpuResource("unassigned_device"));
  EXPECT_EQ(
      "Trying to reclaim unassigned device: unassigned_device.",
      resource_manager_->get_error_message());
}
...
TEST_F(EdgeTpuResourceManagerTest, ExhaustAllEdgeTpu) {
  ...
  for (int i = 0; i < edgetpu_resources.size(); ++i) {
    EXPECT_EQ(kEdgeTpuApiOk, resource_manager_->GetEdgeTpuResource(&edgetpu_resources[i]));
  ...
  EXPECT_EQ(kEdgeTpuApiError, resource_manager_->GetEdgeTpuResource(&another_resource));
...
TEST_F(EdgeTpuResourceManagerTest, MultithreadTest) {
  ...
  auto get_device_assignments = [&generator, &num_devices, &num_threads]() -> std::vector<int> {
  ...
  auto thread_job = [this](const std::string& device_path, int sleep_time) {
    std::unique_ptr<EdgeTpuResource> edgetpu_resource;
    EXPECT_EQ(kEdgeTpuApiOk, resource_manager_->GetEdgeTpuResource(device_path, &edgetpu_resource));
